...
| **#289 (OPEN bug):** reply resolves recipient when the sender was imported under a different local nickname | auto (rust, characterization) + manual/iso (deferred → #291) | `cargo test -p freenet-email-ui _289` — `reply_prefill_addresses_by_display_sender_not_vk_289` (reply To = `m.from`, the sender's alias, not VK) + `lookup_misses_when_contact_imported_under_different_nickname_289` (alias-keyed lookup misses when local label differs). Both pin current buggy behavior and fail loudly when #289 is fixed. Full UI reply round-trip is the deferred iso test in #291. |
