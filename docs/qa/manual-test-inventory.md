...
| **#287 (OPEN bug, red-guarded):** legacy two-party thread (no shared `thread_id`) shows BOTH sides, not one group per sender | auto (red-guard) | offline (`threads.spec.ts`): `a no-thread_id back-and-forth folds into ONE group carrying both senders` — wrapped in `test.fail()`, reproduces the "threaded view only shows one side" report. Seeds a legacy "Picnic this weekend?" exchange onto address2. `group_into_threads` keys legacy mail by `subject\|SENDER` so it splits today; the test flips to a hard failure when grouping merges both sides. |
